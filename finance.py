...
class basicClient:
    ...
    def apiError(self):
        print(
            f"One of api keys for class {self.__class__.__name__} is missing!")
...
class yahooClient(basicClient):
    ...
    def getTicket(self, ticket: str = "", start=dt.datetime.today(), end=dt.datetime.today()):
        result = None
        if super().getTicket(ticket) != "":
...
class kucoinClient(basicClient):
    ...
    def __init__(self, config: Dict[Any, Any] = {}) -> None:
        if len(config) > 0:
            StockApi = namedtuple(
                'StockApi', ['secret', 'passphrase', 'sandbox'])
            try:
    ...
    def getTicket(self, ticket: str = ""):
        result = None
        if super().getTicket(ticket) is not None:
            try:
